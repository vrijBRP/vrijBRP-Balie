...
public class GbaWsCustomPerformanceProfiler extends PerformanceProfiler {

  private static final Logger LOGGER        = LoggerFactory.getLogger(GbaWsCustomPerformanceProfiler.class);
  private static final int    NANO_TO_MILLI = 1_000_000;

  public GbaWsCustomPerformanceProfiler() {
    super(false);
  }
  ...
  protected void addProfile(Profile profile) {
    ...
    if (fil(name)) {
      ...
      long totalTime = profile.getTotalTime() / NANO_TO_MILLI;
      long instances = profile.getNumberOfInstancesEffected();

      Object property = getSession().getActiveSession().getProperty(GbaWsCustomSessionLogger.SQL_STATEMENT);
      if (property != null) {
        getSession().getActiveSession().removeProperty(GbaWsCustomSessionLogger.SQL_STATEMENT);
