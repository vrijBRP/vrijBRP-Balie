...
package nl.procura.gba.jpa.personen.session;
...
public class GbaSessionCustomize implements SessionCustomizer {
      ...
      for (DatabaseMapping mapping : descriptor.getMappings()) {
        if (mapping.isForeignReferenceMapping() && mapping.isCollectionMapping()) {
          ...
          if (colMapping.getContainerPolicy().getContainerClass() == ClassConstants.IndirectList_Class) {
            colMapping.useCollectionClass(UniqueJpaList.class);
          }
        }
      }
    }
