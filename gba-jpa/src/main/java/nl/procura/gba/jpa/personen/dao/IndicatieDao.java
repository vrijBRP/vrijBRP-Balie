...
public class IndicatieDao extends ZaakDao {

  private static final String C_INDICATIE = "cIndicatie";
  ...
  public static final List<ZaakKey> findZaakKeys(ConditionalMap map) {
    ...
    CriteriaWrapper<Indicatie, ZaakKey> w = new CriteriaWrapper<>(Indicatie.class, ZaakKey.class, map);
    ...
    w.getCq().select(w.getBuilder().construct(ZaakKey.class, zaakId, dAanv, dOpn, tOpn)).orderBy(
        getOrder(w, D_IN, T_IN, C_INDICATIE));

    find(w);

    return addZaakType(w.getResultList(), INDICATIE);
  }

  public static final int findCount(ConditionalMap map) {

    CriteriaWrapper<Indicatie, Long> w = new CriteriaWrapper<>(Indicatie.class, Long.class, map);
  ...
  public static final List<Indicatie> find(ConditionalMap map) {

    CriteriaWrapper<Indicatie, Indicatie> w = new CriteriaWrapper<>(Indicatie.class, Indicatie.class, map);

    find(w);

    w.getCq().orderBy(getOrder(w, D_IN, T_IN, C_INDICATIE));

    TypedQuery<Indicatie> query = w.getEm().createQuery(w.getCq());

    return get(query, map);
  }

  private static void find(CriteriaWrapper<Indicatie, ?> w) {

    CriteriaBuilder builder = w.getBuilder();
    Root<Indicatie> table = w.getTable();
    CriteriaQuery query = w.getCq();
    ConditionalMap map = w.getMap();

    List<Predicate> where = new ArrayList<>();

    where.add(builder.greaterThan(table.get(C_INDICATIE), 0));
    ...
    if (map.containsKeys(D_END_TERMIJN, D_AFHAAL_VANAF, D_AFHAAL_TM)) {
      where.add(builder.lessThan(table.get(C_INDICATIE), 0));
    }
