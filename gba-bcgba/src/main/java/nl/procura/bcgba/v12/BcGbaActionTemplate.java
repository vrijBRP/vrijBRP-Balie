...
package nl.procura.bcgba.v12;

import nl.bprbzk.bcgba.v12.AfzenderDE;
import nl.bprbzk.bcgba.v12.BerichtInBase;
import nl.bprbzk.bcgba.v12.BerichtUitBase;
import nl.procura.bcgba.v12.misc.BcGbaCode;
import nl.procura.bcgba.v12.soap.BcGbaSoapHandler;

public class BcGbaActionTemplate {
  ...
  private BcGbaSoapHandler soapHandler    = new BcGbaSoapHandler();
  ...
  protected boolean isResultaat(BcGbaCode code) { // Override
    return false;
  }

  public BcGbaCode getVerwerkingSuccesCode() {
    return BcGbaCode.ONBEKEND;
  }

  public Verwerking getVerwerking() {

    return new Verwerking(BcGbaCode.get(
        getResponseObject().getBcGBABericht().getBerichtResultaatCode()) == getVerwerkingSuccesCode(),
        getResponseObject().getBcGBABericht().getBerichtResultaatCode(),
        getResponseObject().getBcGBABericht().getBerichtResultaatOmschrijving());
  }
  ...
  public String getOutputMessage() {
    ...
    try {
      int code = getResponseObject().getBcGBABericht().getBerichtResultaatCode();
      String oms = getResponseObject().getBcGBABericht().getBerichtResultaatOmschrijving();

      sb.append("algemeen|afzender|" + getResponseObject().getAfzender() + "\n");
      sb.append("algemeen|omschrijving|" + oms + " (" + code + ")\n");
    } catch (Exception e) {
      // ignore
    }
  ...
  public BcGbaSoapHandler getSoapHandler() {
    return soapHandler;
  }

  public void setSoapHandler(BcGbaSoapHandler soapHandler) {
