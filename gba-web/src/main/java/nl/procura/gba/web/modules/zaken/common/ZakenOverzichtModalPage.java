...
public class ZakenOverzichtModalPage<T extends Zaak> extends ZakenModalPage<T> {
    ...
    if (event.isEvent(InitPage.class)) {
      ...
      tabsheet = new ZaakTabsheet<T>(this, getZaak()) {
        ...
        protected void addOptieButtons() {
          ZakenOverzichtModalPage.this.addOptieButtons();
        }

        @Override
        protected void addTabs() {
          ZakenOverzichtModalPage.this.addTabs();
          ZakenOverzichtModalPage.this.addTabs(tabsheet);
        }
      };

      tabsheet.addListener((SelectedTabChangeListener) e -> {
        VaadinUtils.resetHeight(getContainer().getWindow());
  ...
  private void doVerwerken() {
    getParentWindow().addWindow(new BsmVerwerkingWindow<T>(getZaak()) {
