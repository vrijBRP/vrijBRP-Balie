...
public class VogOverzichtBuilder {

  public static void addTab(ZaakTabsheet tabsheet, VogAanvraag zaak) {
    tabsheet.addTab(new VogOverzichtLayout(zaak), "Verklaring omtrent gedrag");

    // Zaken DMS tab
    boolean isZakenDsmAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.BSM_ZAKEN_DMS));
    boolean isZaakDmsAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.ZAKEN_DMS_COVOG));
