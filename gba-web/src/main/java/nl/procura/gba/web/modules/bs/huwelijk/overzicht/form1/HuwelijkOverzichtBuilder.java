...
public class HuwelijkOverzichtBuilder {

  public static void addTab(ZaakTabsheet tabsheet, ZaakDossier zaakDossier) {

    tabsheet.addTab(new HuwelijkOverzichtLayout(to(zaakDossier, DossierHuwelijk.class)), "Huwelijk");

    // Zaken DMS tab
    boolean isZakenDsmAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.BSM_ZAKEN_DMS));
    boolean isZaakDmsAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.ZAKEN_DMS_HUW_GPS));
