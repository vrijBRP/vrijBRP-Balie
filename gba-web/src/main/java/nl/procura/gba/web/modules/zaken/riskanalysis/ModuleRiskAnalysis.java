...
package nl.procura.gba.web.modules.zaken.riskanalysis;
...
@ModuleAnnotation(url = "#zaken.risicoanalyse",
    caption = "Risicoanalyses",
    service = RiskAnalysisService.class,
    zaakTypes = { RISK_ANALYSIS },
    profielActie = ProfielActie.SELECT_CASE_RISK_ANALYSIS)
public class ModuleRiskAnalysis extends ZakenModuleTemplate {
  ...
  public void event(PageEvent event) {
    ...
    if (event.isEvent(InitPage.class)) {
      getPages().getNavigation().goToPage(Page1RiskAnalysis.class);
    }
  }
}
