...
package nl.procura.gba.web.modules.zaken.vog.page1;
...
import nl.procura.gba.web.modules.zaken.vog.page2.Page2Vog;
 ...
 * Overzicht
 */
public class Page1Vog extends ZakenListPage<VogAanvraag> {
  ...
  protected ZaakArgumenten getZaakArgumenten() {
    return new ZaakArgumenten(getPl(), ZaakType.COVOG);
  }
  ...
  protected void setTableColumns(ZakenListTable table) {
    ...
    table.addColumn("Ingevoerd door");
  }

  @Override
  protected void selectTableRecord(ZaakRecord<VogAanvraag> zaakRecord) {
    getNavigation().goToPage(new Page2Vog(zaakRecord.getZaak()));
  }

  @Override
  protected void loadTableRecord(Record record, ZaakRecord<VogAanvraag> zaakRecord) {
    VogAanvraag zaak = zaakRecord.getZaak();
  ...
  @Override
  public void onNew() {
    VogAanvraag zaak = (VogAanvraag) getServices().getVogService().getNewZaak();
    ZaakConfiguratieDialog.of(getApplication(), zaak, getServices(), () -> {
      Page10Vog nextPage = new Page10Vog(zaak);
