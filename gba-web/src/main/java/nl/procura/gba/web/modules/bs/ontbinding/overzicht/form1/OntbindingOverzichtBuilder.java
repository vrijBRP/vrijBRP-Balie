...
public class OntbindingOverzichtBuilder {

  public static void addTab(ZaakTabsheet tabsheet, ZaakDossier zaakDossier) {
    tabsheet.addTab(new OntbindingOverzichtLayout(to(zaakDossier, DossierOntbinding.class)),
        "Ontbinding / beëindiging");

    // Zaken DMS tab
    boolean isZakenDsmAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.BSM_ZAKEN_DMS));
    boolean isZaakDmsAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.ZAKEN_DMS_ONTB_HUW_GPS));
