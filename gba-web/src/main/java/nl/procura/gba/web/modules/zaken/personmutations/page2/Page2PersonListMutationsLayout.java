/*
 * Copyright 2021 - 2022 Procura B.V.
 *
 * In licentie gegeven krachtens de EUPL, versie 1.2
 * U mag dit werk niet gebruiken, behalve onder de voorwaarden van de licentie.
 * U kunt een kopie van de licentie vinden op:
 *
 *   https://github.com/vrijBRP/vrijBRP/blob/master/LICENSE.md
 *
 * Deze bevat zowel de Nederlandse als de Engelse tekst
 *
 * Tenzij dit op grond van toepasselijk recht vereist is of schriftelijk
 * is overeengekomen, wordt software krachtens deze licentie verspreid
 * "zoals deze is", ZONDER ENIGE GARANTIES OF VOORWAARDEN, noch expliciet
 * noch impliciet.
 * Zie de licentie voor de specifieke bepalingen voor toestemmingen en
 * beperkingen op grond van de licentie.
 */

package nl.procura.gba.web.modules.zaken.personmutations.page2;

import nl.procura.diensten.gba.ple.base.BasePL;
import nl.procura.diensten.gba.ple.base.BasePLRec;
import nl.procura.gba.web.components.layouts.GbaVerticalLayout;
import nl.procura.gba.web.services.beheer.personmutations.PersonListActionType;
import nl.procura.vaadin.component.layout.Fieldset;

public class Page2PersonListMutationsLayout extends GbaVerticalLayout {

  private final Page2PersonListMutationsTable table;

  public Page2PersonListMutationsLayout() {
    setSpacing(true);
    table = new Page2PersonListMutationsTable();

    addComponent(new Fieldset("Elementen"));
    addExpandComponent(table);
  }

  public void update(PersonListActionType action, BasePL pl, BasePLRec record) {
    table.update(action, pl, record);
  }

  public Page2PersonListMutationsTable getTable() {
    return table;
  }
}
