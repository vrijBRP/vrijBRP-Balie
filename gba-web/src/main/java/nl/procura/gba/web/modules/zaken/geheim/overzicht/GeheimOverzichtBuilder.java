...
public class GeheimOverzichtBuilder {

  public static void addTab(ZaakTabsheet tabsheet, GeheimAanvraag zaak) {
    tabsheet.addTab(new GeheimOverzichtLayout(zaak), "Verstrekkingsbeperking");

    // Zaken DMS tab
    boolean isZakenDsmAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.BSM_ZAKEN_DMS));
    boolean isZaakDmsAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.ZAKEN_DMS_VERSTREK));
