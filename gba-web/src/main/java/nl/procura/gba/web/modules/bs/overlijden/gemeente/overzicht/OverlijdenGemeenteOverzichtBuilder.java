...
public class OverlijdenGemeenteOverzichtBuilder {

  public static void addTab(ZaakTabsheet tabsheet, ZaakDossier zaakDossier) {
    tabsheet.addTab(new OverlijdenGemeenteOverzichtLayout((DossierOverlijdenGemeente) zaakDossier),
        "Overlijden in gemeente");

    // Zaken DMS tab
    boolean isZakenDsmAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.BSM_ZAKEN_DMS));
    boolean isZaakDmsAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.ZAKEN_DMS_OVERL_GEM));
