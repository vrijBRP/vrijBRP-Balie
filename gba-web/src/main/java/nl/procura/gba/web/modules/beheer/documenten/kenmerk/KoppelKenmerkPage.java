...
public class KoppelKenmerkPage<K extends KoppelbaarAanDocumentKenmerk> extends KoppelPage {

  private final KoppelForm      form;
  private final List<K>         koppelList;
  private KoppelKenmerkTabel<K> tabel = null;
  private final String          type;

  public KoppelKenmerkPage(List<K> koppelList, String type) {
    ...
    setInfo("Klik één keer op de regel om deze te selecteren, dubbelklik om de status te wijzigen. <br>"
        + "De status is 'Gekoppeld' als <b>alle</b> geselecteerde " + type + " gekoppeld zijn aan het kenmerk.");
  ...
  protected void allesKoppelActie(KoppelActie koppelActie) {
    ...
    new KoppelProcedure(tabel, koppelActie, type, true) {
      ...
      public void koppel(KoppelActie koppelActie, boolean wholeTable) {
        KoppelKenmerkPage.this.koppel(koppelActie, wholeTable);
      }
    };
  }
  ...
  protected void koppel(KoppelActie koppelActie, boolean heleTabel) {
    ...
    List<DocumentKenmerk> printOpties = getSelectedPrintoptions(heleTabel);

    koppelActieDocumentKenmerk(koppelActie, koppelList, printOpties);
  ...
  private List<DocumentKenmerk> getSelectedPrintoptions(boolean heleTabel) {

    if (heleTabel) {
      return tabel.getAllValues(DocumentKenmerk.class);
    }

    return tabel.getSelectedValues(DocumentKenmerk.class);
  }
  ...
  private void setTable() {
    ...
    tabel = new KoppelKenmerkTabel<K>(koppelList) {
