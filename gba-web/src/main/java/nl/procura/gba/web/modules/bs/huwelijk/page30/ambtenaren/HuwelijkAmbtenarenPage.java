...
public class HuwelijkAmbtenarenPage extends ButtonPageTemplate {
  ...
  public HuwelijkAmbtenarenPage(AmbtenarenForm form1, DossierPersoon dossierPersoon) {
  ...
  class Table1 extends GbaTable {
    ...
    public void setRecords() {
      ...
      List<HuwelijksAmbtenaar> ambtenaren = getServices().getHuwelijkService().getHuwelijksAmbtenaren(
          geldigheidField.getValue());
      for (HuwelijksAmbtenaar l : ambtenaren) {
        add(l);
      }
    ...
    private void selectRecord(Record record) {
      ...
      if (bsn > 0) {
        ...
        if (!result.getBasisPLWrappers().isEmpty()) {
