...
public class ReisdocumentOverzichtBuilder {

  public static void addTab(ZaakTabsheet tabsheet, ReisdocumentAanvraag zaak) {
    tabsheet.addTab(new ReisdocumentOverzichtLayout(zaak), "Reisdocument");

    // Zaken DMS tab
    boolean isZakenDsmAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.BSM_ZAKEN_DMS));
    boolean isZaakDmsAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.ZAKEN_DMS_REISDOC));
