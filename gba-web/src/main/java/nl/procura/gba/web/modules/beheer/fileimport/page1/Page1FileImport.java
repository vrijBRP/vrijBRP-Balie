...
public class Page1FileImport extends NormalPageTemplate {

  private GbaTable table = null;

  public Page1FileImport() {
      ...
      table = new GbaTable() {
        ...
        public void setRecords() {
          ...
          for (FileImport fileImport : fileImports) {
            ...
            r.addValue(String.format("%s (%s)",
                fileImport.getName(),
                Optional.of(service.countRecords(fileImport))
                    .map(c -> String.format("%d / %d",
                        c.getNewRecords(), c.getTotalRecords()))
                    .orElse("0 / 0")));
            r.addValue(FileImportType.getById(fileImport.getTemplate())
                .map(FileImportType::getDescr)
                .orElse("Onbekend type"));
            r.addValue(fileImport.isClosed() ? "Ja" : "Nee");
          }
        }
      };
  ...
  public void onDelete() {
    new DeleteRecordsFromTable(table) {
      ...
      public void deleteRecord(Record record) {
        Page1FileImport.this.deleteRecord(record);
      }
    };
  }
