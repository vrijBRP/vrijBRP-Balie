...
public class Page40OmzettingTable extends GbaTable {

  final private List<DossierPersoon> getuigenLijst = new ArrayList<>();

  public Page40OmzettingTable(DossierOmzetting dossierOmzetting) {

    List<DossierPersoon> set = dossierOmzetting.getGetuigen();
    getuigenLijst.addAll(set);
    int aantal = dossierOmzetting.getHuwelijksLocatie().getLocatieSoort().getAantalGetuigenMax();
  ...
  public void setRecords() {
    ...
    for (DossierPersoon person : BsPersoonUtils.sort(getuigenLijst)) {
      ...
      if (person.isVolledig()) {
        ...
        r.addValue(person.getNaam().getPred_adel_voorv_gesl_voorn());
