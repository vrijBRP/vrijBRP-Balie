...
public class RijbewijsOverzichtBuilder {

  public static void addTab(final ZaakTabsheet tabsheet, RijbewijsAanvraag zaak) {
    tabsheet.addTab(new RijbewijsOverzichtLayout(zaak) {

      @Override
      protected void success(String message) {
        new Message(tabsheet.getWindow(), message, Message.TYPE_SUCCESS);
      }
    }, "Rijbewijs");

    // Zaken DMS tab
    boolean isZakenDsmAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.BSM_ZAKEN_DMS));
    boolean isZaakDmsAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.ZAKEN_DMS_RIJBEWIJS));
