/*
 * Copyright 2021 - 2022 Procura B.V.
 *
 * In licentie gegeven krachtens de EUPL, versie 1.2
 * U mag dit werk niet gebruiken, behalve onder de voorwaarden van de licentie.
 * U kunt een kopie van de licentie vinden op:
 *
 *   https://github.com/vrijBRP/vrijBRP/blob/master/LICENSE.md
 *
 * Deze bevat zowel de Nederlandse als de Engelse tekst
 *
 * Tenzij dit op grond van toepasselijk recht vereist is of schriftelijk
 * is overeengekomen, wordt software krachtens deze licentie verspreid
 * "zoals deze is", ZONDER ENIGE GARANTIES OF VOORWAARDEN, noch expliciet
 * noch impliciet.
 * Zie de licentie voor de specifieke bepalingen voor toestemmingen en
 * beperkingen op grond van de licentie.
 */

package nl.procura.gba.web.modules.bs.ontbinding.page30;

import java.io.Serializable;
import java.lang.annotation.ElementType;
import java.util.Date;

import nl.procura.vaadin.annotation.field.Field;
import nl.procura.vaadin.annotation.field.FormFieldFactoryBean;
import nl.procura.vaadin.annotation.field.TextArea;
import nl.procura.vaadin.component.field.ProDateField;
import nl.procura.vaadin.component.field.ProTextArea;

@FormFieldFactoryBean(accessType = ElementType.FIELD)
public class Page30OntbindingBean3 implements Serializable {

  public static final String DATUM_VERKLARING    = "datumVerklaring";
  public static final String DATUM_ONDERTEKENING = "datumOndertekening";
  public static final String ONDERTEKEND_DOOR    = "ondertekendDoor";

  @Field(customTypeClass = ProDateField.class,
      caption = "Verklaring ontvangen op",
      required = true,
      width = "100px")
  private Date datumVerklaring = null;

  @Field(customTypeClass = ProDateField.class,
      caption = "Ondertekend op",
      required = true,
      width = "100px")
  private Date   datumOndertekening = null;
  @Field(customTypeClass = ProTextArea.class,
      caption = "Ondertekend door partijen en",
      required = true,
      width = "400px")
  @TextArea(nullRepresentation = "",
      maxLength = 490,
      rows = 3)
  private String ondertekendDoor    = null;

  public Date getDatumOndertekening() {
    return datumOndertekening;
  }

  public void setDatumOndertekening(Date datumOndertekening) {
    this.datumOndertekening = datumOndertekening;
  }

  public Date getDatumVerklaring() {
    return datumVerklaring;
  }

  public void setDatumVerklaring(Date datumVerklaring) {
    this.datumVerklaring = datumVerklaring;
  }

  public String getOndertekendDoor() {
    return ondertekendDoor;
  }

  public void setOndertekendDoor(String ondertekendDoor) {
    this.ondertekendDoor = ondertekendDoor;
  }
}
