...
package nl.procura.gba.web.rest.v1_0.zaak.zoeken.indicatie;
...
public class GbaRestIndicatieHandler extends GbaRestElementHandler {

  public GbaRestIndicatieHandler(Services services) {
    super(services);
  }

  public void convert(GbaRestElement gbaZaak, Zaak zaakParm) {

    IndicatieAanvraag zaak = (IndicatieAanvraag) zaakParm;
    IndicatieActie actie = zaak.getActie();
    PlAantekeningIndicatie plIndicatie = zaak.getIndicatie();

    GbaRestElement indicatie = gbaZaak.add(INDICATIE);

    add(indicatie, ACTIE, actie.getCode(), actie.getAfkort(), actie.getOmschrijving());
    add(indicatie, INDICATIE, plIndicatie.getIndicatie(), plIndicatie.getOmschrijving());
    add(indicatie, INDICATIE_PROBEV, plIndicatie.getProbev());
    add(indicatie, TOELICHTING, zaak.getInhoud());
