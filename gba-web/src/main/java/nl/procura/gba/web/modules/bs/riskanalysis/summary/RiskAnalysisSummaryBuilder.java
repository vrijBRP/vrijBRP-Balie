...
public class RiskAnalysisSummaryBuilder {

  public static void addTab(ZaakTabsheet tabsheet, ZaakDossier zaakDossier) {

    DossierRiskAnalysis dossierRiskAnalysis = to(zaakDossier, DossierRiskAnalysis.class);
    tabsheet.addTab(new RiskAnalysisSummaryLayout(dossierRiskAnalysis), "Risicoanalyse");

    // Zaken DMS tab
    boolean isZakenDsmAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.BSM_ZAKEN_DMS));
    boolean isZaakDmsAan = isTru(tabsheet.getApplication().getParmValue(ParameterConstant.ZAKEN_DMS_ONDERZOEK));
