...
public class SelectieMelding extends ServiceMelding {

  private Selectie selectie;

  public SelectieMelding(Selectie selectie, int count) {
    this.selectie = selectie;
    setId("Selectie: " + selectie.getSelectie());
    setAdminOnly(false);
    setCategory(WORK);
    setSeverity(WARNING);
    setMelding(String.format("Selectie: %s %d record(s) gevonden", selectie.getSelectie(), count));
  }
