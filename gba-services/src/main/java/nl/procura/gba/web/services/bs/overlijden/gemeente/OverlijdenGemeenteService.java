...
public class OverlijdenGemeenteService extends OverlijdenService {

  public OverlijdenGemeenteService() {
    super("Overlijden in gemeente", ZaakType.OVERLIJDEN_IN_GEMEENTE);
  }

  @Override
  public Zaak getNewZaak() {
    return aanvullenZaak(new DossierOverlijdenGemeente().getDossier());
  }

  @Override
  @ThrowException("Het is niet mogelijk om de zaak op te vragen")
  public Dossier getStandardZaak(Dossier zaak) {
    DossierOverlijdenGemeente zaakDossier = getServices().getDossierService()
        .getZaakDossier(zaak, DossierOverlijdenGemeente.class);
    zaakDossier.setGemeenteOverlijden(PLAATS.get(zaakDossier.getcOverlGem()));
