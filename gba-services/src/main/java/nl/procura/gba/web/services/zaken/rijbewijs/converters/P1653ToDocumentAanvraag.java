...
import nl.procura.rdw.processen.p1653.f02.AANVRRYBKGEG;
import nl.procura.rdw.processen.p1653.f02.AANVRRYBKRT;
import nl.procura.rdw.processen.p1653.f02.ADRESNATPGEG;
import nl.procura.rdw.processen.p1653.f02.CATAANRYBGEG;
import nl.procura.rdw.processen.p1653.f02.GESCHVERKLGEG;
import nl.procura.rdw.processen.p1653.f02.NATPERSOONGEG;
import nl.procura.rdw.processen.p1653.f02.RYBGEG;
import nl.procura.rdw.processen.p1653.f02.RYVVERKLGEG;
import nl.procura.rdw.processen.p1653.f02.STATRYBKGEG;
import nl.procura.validation.Anr;
import nl.procura.validation.Bsn;

public class P1653ToDocumentAanvraag {

  private final static Logger LOGGER = LoggerFactory.getLogger(P1653ToDocumentAanvraag.class.getName());
  ...
  public static RijbewijsAanvraagAntwoord get(AANVRRYBKRT process, Services services) {
    ...
    for (CATAANRYBGEG catgeg : process.getCataanrybtab().getCataanrybgeg()) {
      ...
      String rStatus2 = ryvgeg.getStatusrverkl();

      if (fil(rStatus2)) {
        rStatus2 += ": " + RijbewijsStatusRijvaardigheid.get(ryvgeg.getStatusrverkl()).getOms();
      }

      cat_gegevens.getRijv_gegevens().setStatus(rStatus2);
