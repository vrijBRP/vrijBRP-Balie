...
public class TestCOVOGVerzend {

  public TestCOVOGVerzend() {
    String endpoint = "https://covogacc.minjus.gemnet.nl/webservices/AanvraagNpAPI";
    VogSoapHandler soapHandler = new VogSoapHandler(endpoint, "<relatiecode>", "<idcode>");

    try {
      System.out.println("RETURN: " + soapHandler.send(new DummyCovogAanvraag()));
    } catch (Exception e) {
      e.printStackTrace();
    } finally {
