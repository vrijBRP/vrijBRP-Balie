...
public class ActionContrBsnIdenGeg extends BvBsnActionTemplate implements BvBsnActionInterface {

  private final static Logger LOGGER     = LoggerFactory.getLogger(ActionContrBsnIdenGeg.class);
  private String              bsn        = "";
  private IDGegevens          idGegevens = null;

  public ActionContrBsnIdenGeg(AfzenderDE afzender, String indicatieEindgebruiker, String bsn,
  ...
  public String getOutputMessage() {
    ContrBsnIdenGegBU a = (ContrBsnIdenGegBU) getResponseObject();
    StringBuilder sb = new StringBuilder();

    sb.append(super.getOutputMessage());
    try {
      ContrBsnIdenGegResultaatDE geg = a.getResultaat().getContrBsnIdenGegResultaatDE().get(0);
      String newLine = "\n";

      sb.append("Actie|melding|");
      sb.append(geg.getResultaatOmschrijving());
      sb.append(" (");
      sb.append(geg.getResultaatCode());
      sb.append(")");
      sb.append(newLine);
    } catch (Exception e) {
    ...
    if (a.getResultaat() != null) {
      ...
      GebruikerIdenGegAntwoordDE g = a.getResultaat().getContrBsnIdenGegResultaatDE().get(0).getIdenGegAntwoord();
  ...
  public BerichtInBase getRequestObject() {
    ...
    ContrBsnIdenGegBI berichtIn = new ContrBsnIdenGegBI();
    ...
    newIdGegevens.setAanduidingBijHuisnummer(getIdGegevens().getAanduidingBijHuisnummer());
    newIdGegevens.setGeboortedatum(getIdGegevens().getGeboortedatum());
    newIdGegevens.setGeboorteland(getIdGegevens().getGeboorteland());
    newIdGegevens.setGeboorteplaats(getIdGegevens().getGeboorteplaats());
    newIdGegevens.setGemeenteVanInschrijving(getIdGegevens().getGemeenteVanInschrijving());
    newIdGegevens.setGeslachtsaanduiding(getIdGegevens().getGeslachtsaanduiding());
    newIdGegevens.setGeslachtsnaam(getIdGegevens().getGeslachtsnaam());
    newIdGegevens.setHuisletter(getIdGegevens().getHuisletter());
    newIdGegevens.setHuisnummer(getIdGegevens().getHuisnummer());
    newIdGegevens.setHuisnummertoevoeging(getIdGegevens().getHuisnummertoevoeging());
    newIdGegevens.setLandVanwaarIngeschreven(getIdGegevens().getLandVanwaarIngeschreven());
    newIdGegevens.setLocatiebeschrijving(getIdGegevens().getLocatiebeschrijving());
    newIdGegevens.setPostcode(getIdGegevens().getPostcode());
    newIdGegevens.setStraatnaam(getIdGegevens().getStraatnaam());
    newIdGegevens.setVoornamen(getIdGegevens().getVoornamen());
    newIdGegevens.setVoorvoegselGeslachtsnaam(getIdGegevens().getVoorvoegselGeslachtsnaam());
    ContrBsnIdenGegVraagDE vraag = new ContrBsnIdenGegVraagDE();

    vraag.setVraagnummer(1);
    vraag.setBurgerServiceNr(Long.parseLong(getBsn()));
    vraag.setIdenGeg(newIdGegevens);
    ArrayOfContrBsnIdenGegVraagDE vragen = new ArrayOfContrBsnIdenGegVraagDE();

    vragen.getContrBsnIdenGegVraagDE().add(vraag);
  ...
  public int getVerwerkingSuccesCode() {
    return CODES.ContrBsnIdenGeg.VERWERKING_SUCCESS;
  }

  public boolean isGegevensGevonden() {
    try {
      return ((ContrBsnIdenGegBU) getResponseObject()).getResultaat().getContrBsnIdenGegResultaatDE().get(
          0).getResultaatCode() == CODES.ContrBsnIdenGeg.SUCCESS;
    } catch (Exception e) {
      LOGGER.debug(e.toString());
      return false;
    }
